...
class EvRaindrop_v1(nn.Module):
    ...
    def build_hypergraph(self, static_features):
        """为每个批次生成独立的超边索引，避免跨批次混合"""
    ...
    def forward(self, src, static, times, lengths):
        ...
        adj = self.global_structure.to(src.device) if self.global_structure is not None else torch.ones([128, 128]).cuda()
        ...
        for unit in range(batch_size):
            ...
            if static is not None and self.static and hyperedge_list is not None:
                updated_core_feats_layer2 = stepdata[:self.d_inp]  
                updated_static_feats_layer2 = stepdata[self.d_inp:]  
                
                interacted_static_layer2 = updated_static_feats_layer2.mean(dim=1)  
                
                interacted_static[unit] = interacted_static[unit] + interacted_static_layer2  
                interacted_static[unit] = F.relu(interacted_static[unit])
                
                stepdata = updated_core_feats_layer2
            else:
